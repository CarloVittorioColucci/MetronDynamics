...
class Qwiic:
    def __init__(self, i2c=None, address: int = 0x30, native: bool = False, scl: int = 9, sda: int = 8):
        """
        Initialize the Qwiic interface.

        :param i2c: Initialized machine.I2C object (optional, overrides scl/sda)
        :param address: I2C address of the device
        :param native: Use native initialization instead of I2C
        :param scl: SCL pin number (default 9 for Soldered NULA/easyC connector)
        :param sda: SDA pin number (default 8 for Soldered NULA/easyC connector)
        """
        if i2c is not None:
            self.i2c = i2c
        else:
            if uname().sysname in ("esp32", "esp8266", "Soldered Dasduino CONNECTPLUS"):
                ...
                raise Exception(
                    "Board not recognized, enter Qwiic I2C pins manually")
    ...
    def initialize_native(self):
        ...
        raise NotImplementedError(
            "Subclasses must implement initialize_native()")
